        ...
        */
        ...
        stage('correlate-delta-and-archive') {
            steps {
                    ...
                    sh '''#!/bin/bash
                        source ./env-setup/8.0_env_setup.sh
                        ./gpgpu-sim_simulations/util/job_launching/get_stats.py -R -K -k -B rodinia_2.0-ft -C QV100 -A > stats-per-kernel-8.0.csv'''
                    ...
                    sh './gpgpu-sim_simulations/util/plotting/merge-stats.py -c ./gpgpu-sim-results-repo/${JOB_NAME}/stats-per-app-8.0.csv,./stats-per-app-8.0.csv -R > per-app-merge-8.0.csv'
                    sh 'PLOTDIR="jenkins/${JOB_NAME}" &&\
                        ./gpgpu-sim_simulations/util/plotting/plot-get-stats.py -c per-app-merge-8.0.csv -P cuda-8.0 &&\
                        ./gpgpu-sim_simulations/util/plotting/merge-stats.py -c ./gpgpu-sim-results-repo/${JOB_NAME}/stats-per-kernel-8.0.csv,./stats-per-kernel-8.0.csv -R > per-kernel-merge-8.0.csv &&\
                        ./gpgpu-sim_simulations/util/plotting/plot-correlation.py -H ./gpgpu-sim_simulations/run_hw/QUADRO-V100/device-0/9.1/ -c per-kernel-merge-8.0.csv -p cuda-8.0 | grep -B 1 "Correl=" | tee correl.8.0.txt &&\
